...
本仓库的设计初衷，是将你的大语言模型从一个“只会罗列数据的聊天机器人”，升级为一位**守纪律的专业投资研究员**。使用本框架，你能获得以下实际增益：

- **聚焦“企业所有权”视角：** 强迫 AI 在探讨价格之前，先深度评估企业的护城河持久性、管理层的坦诚度以及资本配置的质量。
- **第一手信息优先 (Primary-source first)：** 帮您过滤掉市场上的二手噪音。引导指令强制 AI 优先摄取 10-K/20-F 财报、Proxy 管理层明细以及电话会议文字录。
- **高信噪比的“快速否定”文化：** 框架赋予了 AI 随时给出 `放弃 (Pass)` 或 `超出能力圈保护自己 (Outside Competence)` 的权力，快速帮你滤掉平庸的生意。
- **随取随用的行业护城河手册 (Playbooks)：** 拒绝拿着锤子找钉子的通用分析模板。它会根据不同行业（如保险业的浮存金、SaaS 业的转换成本）加载针对性的心智模型。
- **跨市场研究的极速启动：** 仓库内置的 Python 脚手架工具，能瞬间为您提取（美股 SEC、港股 HKEX、A 股沪/深）官方信源的唯一定位入口，解决跨市场分析的“起步难”问题。
